...
export default function CartSidebar() {
  const { hideRightSidebar, changeCartState, cart } = useContext(CartContext);
  const { addSales, sellProduct } = useContext(InventoryContext);

  const checkoutItems = () => {
    console.log(cart);
    cart?.forEach((item) => {
      addSales(item, item.leftQuantity);
      sellProduct(item, item.leftQuantity);
    });
  };
  ...
  return (
    ...
    >
      ...
      <div className="w-1/4 bg-white">
        ...
        <button className="absolute right-10 bottom-10" onClick={checkoutItems}>
          checkout
        </button>
      </div>
    </div>
